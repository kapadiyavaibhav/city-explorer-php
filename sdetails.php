...
if ($id === 0) { die('Invalid school ID'); }
$res = mysqli_query($conn, "SELECT * FROM schools WHERE id = $id");
$school = mysqli_fetch_assoc($res);
if (!$school) { die('School not found'); }

$school['departments'] = $school['departments'] ? explode(',', $school['departments']) : [];
$school['facilities'] = $school['facilities'] ? explode(',', $school['facilities']) : [];
$school['gallery'] = $school['gallery'] ? explode(',', $school['gallery']) : [];
$school['faculty'] = $school['faculty'] ? array_map(function($f){
    list($n,$d,$e)=array_pad(explode('|',$f),3,'');
    return ['name'=>$n,'dept'=>$d,'exp'=>$e];
}, explode(',',$school['faculty'])) : [];
$school['scholarships'] = $school['scholarships'] ? explode(',', $school['scholarships']) : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .school-logo { width: 150px; height: 150px; border-radius: 50%; object-fit: cover; margin-right: 28px; border: 3px solid #e0e7ef; background: #fff; }
        .school-title { font-size: 2.3rem; color: #2d6ca2; letter-spacing: 1px; margin-bottom: 4px; }
        .school-address a { color: #3887ff; text-decoration: none; font-size: 1.08rem; }
        ...
        /* Navbar tabs style (same as PG/College details) */
        .navbar-tabs {
            display: flex;
            gap: 34px;
            font-size: 1.08rem;
...
<div class="containerr">
    <div class="headerr">
        <img src="images/<?= htmlspecialchars($school['img']) ?>" alt="School Logo" class="school-logo">
        <div>
            <div class="school-title"><?= htmlspecialchars($school['name']) ?></div>
            <div class="school-address">
                <?= htmlspecialchars($school['address']) ?> &#x1F5FA;
                <a href="https://maps.google.com/?q=<?= urlencode($school['name']) ?>,<?= urlencode($school['city_name']) ?>" target="_blank">View on Map</a>
            </div>
            <ul class="info-list">
                <li>📞 <b>Contact:</b> <a href="tel:<?= htmlspecialchars($school['contact']) ?>" onclick="event.stopPropagation();"><?= htmlspecialchars($school['contact']) ?></a></li>
                <li>✉️ <b>Email:</b> <a href="mailto:<?= htmlspecialchars($school['email']) ?>"><?= htmlspecialchars($school['email']) ?></a></li>
                <li>🌐 <b>Website:</b> <?php if($school['website']): ?><a href="<?= htmlspecialchars($school['website']) ?>" target="_blank"><?= htmlspecialchars($school['website']) ?></a><?php endif; ?></li>
                <li><b>Established:</b> <?= htmlspecialchars($school['established']) ?></li>
                <li><b>Grades:</b> <?= htmlspecialchars($school['grades']) ?></li>
            </ul>
        </div>
    </div>
    ...
    <nav>
        <div class="navbar-tabs" id="navbarTabs">
            ...
            <button data-tab="deptTab">Departments & Facilities</button>
    ...
    <div class="section tab-section active" id="facultyTab">
        ...
        <div class="faculty-list">
            <?php foreach($school['faculty'] as $fac): ?>
    ...
    <div class="section tab-section" id="deptTab">
        <div class="section-title">Departments</div>
        <ul class="departments-list">
            <?php foreach($school['departments'] as $dept): ?>
                <li>• <?= htmlspecialchars($dept) ?></li>
            <?php endforeach; ?>
        </ul>
        <div class="section-title" style="margin-top:22px;">Facilities</div>
        <ul class="facilities-list">
            <?php foreach($school['facilities'] as $fac): ?>
                <li><?= htmlspecialchars($fac) ?></li>
            <?php endforeach; ?>
            <?php if($school['hostel']) echo '<li>Hostel</li>'; ?>
            <?php if($school['transport']) echo '<li>Transport</li>'; ?>
            <?php if($school['library']) echo '<li>Library</li>'; ?>
            <?php if($school['sports']) echo '<li>Sports</li>'; ?>
            <?php if($school['cafeteria']) echo '<li>Cafeteria</li>'; ?>
            <?php if($school['disabled_friendly']) echo '<li>Disabled Friendly</li>'; ?>
        </ul>
    </div>
    <div class="section tab-section" id="admissionTab">
        <div class="section-title">Admission Process</div>
        <div class="description"><?= htmlspecialchars($school['admission_process']) ?></div>
        <div class="section-title" style="margin-top:22px;">Scholarships</div>
        <ul class="scholarships-list">
            <?php foreach($school['scholarships'] as $sch): ?>
    ...
    <div class="section tab-section" id="galleryTab">
        ...
        <ul class="gallery-list">
            <?php foreach($school['gallery'] as $img): ?>
                <li>
