...
$sql = "SELECT * FROM pgs WHERE city_name='$city'" . ($type ? " AND cat_name='$type'" : "");
...
<head>
    ...
    <title>City Explorer - Paying Guests</title>
    ...
    .pg-map-link, .contact-list a {
      pointer-events: auto;
      cursor: pointer;
      position: relative;
      z-index: 2;
    }
    .listing-card *:not(.pg-map-link):not(.contact-list a) {
      z-index: 2;
    }
    ...
    function filterTableByName() {
        ...
        if (cards.length) {
            cards.forEach(function(card) {
                var nameElem = card.querySelector(".pg-title");
                if (!nameElem) return;
    ...
    <div class="listing-container">
        <?php foreach($records as $r): ?>
            <div class="listing-card-wrapper">
                <div class="listing-card" onclick="window.location.href='pdetails.php?id=<?php echo $r['id']; ?>'">
                    <a href="pdetails.php?id=<?= $r['id']; ?>" class="overlay-link" tabindex="-1" aria-label="More details"></a>
                    ...
                    <div class="listing-center">
                        <div class="pg-title"><?php echo htmlspecialchars($r['name']); ?></div>
                        <div class="pg-address-row">
                            <span class="pg-address"><?php echo htmlspecialchars($r['address']); ?></span>
                            <a class="pg-map-link" href="https://maps.google.com/?q=<?php echo urlencode($r['name']); ?>" target="_blank" onclick="event.stopPropagation();">View on Map</a>
                        </div>
                    ...
                    <div class="listing-right">
                        ...
                        <div class="field-row">
                            ...
                            <?php echo htmlspecialchars($r['rooms']); ?>
                        </div>
                        <div class="field-row">
                            <span class="field-label">Room Types:</span>
                            <?php echo htmlspecialchars($r['room_types']); ?>
                        </div>
                        <div class="field-row">
