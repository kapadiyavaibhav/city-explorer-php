...
if (isset($_POST['remove_gallery_img']) && isset($_POST['id'])) {
    ...
    $res = mysqli_query($conn, "SELECT gallery FROM pgs WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    $gallery = str2arr($row['gallery']);
    $gallery = array_filter($gallery, fn($img) => $img !== $remove_img);
    $gallery_str = arr2str($gallery);
    mysqli_query($conn, "UPDATE pgs SET gallery='$gallery_str' WHERE id=$id");
...
if (isset($_POST['save'])) {
    ...
    $fields = [
        ...
        'established','rooms','room_types','facilities','faculty','admission_process',
    ...
    $old_gallery = [];
    if ($id) {
        $fetch = mysqli_query($conn, "SELECT img,gallery FROM pgs WHERE id=$id");
    ...
    foreach ($fields as $f) {
        ...
        elseif (in_array($f, ['room_types','facilities','scholarships']))
    ...
    if ($id) {
        ...
        $sql = "UPDATE pgs SET ".implode(',',$sets)." WHERE id=$id";
        ...
        $sql = "INSERT INTO pgs ($fieldstr) VALUES ('$values')";
        mysqli_query($conn, $sql);
...
if (isset($_GET['del'])) {
    $id = intval($_GET['del']);
    $res = mysqli_query($conn, "SELECT img,gallery FROM pgs WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    if ($row) {
        if (!empty($row['img'])) {
    ...
    mysqli_query($conn, "DELETE FROM pgs WHERE id=$id");
...
if ($edit_id) {
    $res = mysqli_query($conn, "SELECT * FROM pgs WHERE id=$edit_id");
    $row = mysqli_fetch_assoc($res);
    if ($row) $editing = true;
}

$res = mysqli_query($conn, "SELECT * FROM pgs ORDER BY id DESC");
$rows = [];
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" id="pgForm">
        ...
        <!-- Add checkbox code for room_types as needed -->
<label>Room Types</label>
    ...
    <div class="yesno inline"><input type="checkbox" name="room_types[]" value="Non AC" <?= checked_roomtype('Non AC', $selected_room_types) ?>> Non AC</div>
        ...
        <div class="form-section">
            ...
            <input type="text" name="facilities[]" value="<?= htmlspecialchars($editing?str_replace(',',',',$row['facilities']??''):'') ?>" placeholder="Wi-Fi,Mess,Power Backup">
        </div>
        <div class="form-section">
        ...
        <div class="form-section">
            ...
            <div class="yesno inline"><input type="checkbox" name="hostel" value="Hostel" <?= $editing && $row['hostel']=='Hostel'?'checked':'' ?>> Food</div>
            <div class="yesno inline"><input type="checkbox" name="transport" value="Transport" <?= $editing && $row['transport']=='Transport'?'checked':'' ?>> Kitchen</div>
    ...
    ?>
    <h2>All P.G.s</h2>
    <input type="text" id="searchBar" placeholder="Search by P.G. Name..." onkeyup="filterTable()">
    <table id="pgTable">
        <tr>
