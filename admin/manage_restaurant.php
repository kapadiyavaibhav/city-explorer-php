...
if (isset($_POST['remove_gallery_img']) && isset($_POST['id'])) {
    ...
    $res = mysqli_query($conn, "SELECT gallery FROM restaurants WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    $gallery = str2arr($row['gallery']);
    $gallery = array_filter($gallery, fn($img) => $img !== $remove_img);
    $gallery_str = arr2str($gallery);
    mysqli_query($conn, "UPDATE restaurants SET gallery='$gallery_str' WHERE id=$id");
...
if (isset($_POST['save'])) {
    ...
    $fields = [
        ...
        'payment_options','description','gallery'
    ];
    $data = [];
    $old_img = '';
    $old_gallery = [];
    if ($id) {
        $fetch = mysqli_query($conn, "SELECT img,gallery FROM restaurants WHERE id=$id");
    ...
    if ($id) {
        ...
        $sql = "INSERT INTO restaurants ($fieldstr) VALUES ('$values')";
        mysqli_query($conn, $sql);
...
if (isset($_GET['del'])) {
    $id = intval($_GET['del']);
    $res = mysqli_query($conn, "SELECT img,gallery FROM restaurants WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    if ($row) {
        if (!empty($row['img'])) {
...
if ($edit_id) {
    $res = mysqli_query($conn, "SELECT * FROM restaurants WHERE id=$edit_id");
    $row = mysqli_fetch_assoc($res);
    if ($row) $editing = true;
}
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" id="restaurantForm">
        ...
        <div class="form-section">
            ...
            <input type="text" name="cuisines[]" value="<?= htmlspecialchars($editing?str_replace(',','',$row['cuisines']??''):'') ?>" placeholder="Indian,Chinese,Italian">
        </div>
    ...
    <input type="text" id="searchBar" placeholder="Search by Restaurant Name..." onkeyup="filterTable()">
