...
if (isset($_POST['remove_gallery_img']) && isset($_POST['id'])) {
    ...
    $res = mysqli_query($conn, "SELECT gallery FROM schools WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    $gallery = str2arr($row['gallery']);
    $gallery = array_filter($gallery, fn($img) => $img !== $remove_img);
    $gallery_str = arr2str($gallery);
    mysqli_query($conn, "UPDATE schools SET gallery='$gallery_str' WHERE id=$id");
...
if (isset($_POST['save'])) {
    ...
    $old_gallery = [];
    if ($id) {
        $fetch = mysqli_query($conn, "SELECT img,gallery FROM schools WHERE id=$id");
    ...
    if ($id) {
        ...
        $sql = "INSERT INTO schools ($fieldstr) VALUES ('$values')";
        mysqli_query($conn, $sql);
...
if (isset($_GET['del'])) {
    $id = intval($_GET['del']);
    $res = mysqli_query($conn, "SELECT img,gallery FROM schools WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    if ($row) {
        if (!empty($row['img'])) {
    ...
    mysqli_query($conn, "DELETE FROM schools WHERE id=$id");
...
if ($edit_id) {
    $res = mysqli_query($conn, "SELECT * FROM schools WHERE id=$edit_id");
    $row = mysqli_fetch_assoc($res);
    if ($row) $editing = true;
}

$res = mysqli_query($conn, "SELECT * FROM schools ORDER BY id DESC");
$rows = [];
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" id="schoolForm">
        ...
        <input type="text" name="grades" value="<?= htmlspecialchars($editing?$row['grades']:'') ?>">

        <!-- Add your courses, departments, etc. fields as needed -->

        <div class="form-section">
            <label>Departments <span class="small">(comma-separated)</span></label>
            <input type="text" name="departments[]" value="<?= htmlspecialchars($editing?str_replace(',','',$row['departments']??''):'') ?>" placeholder="Mathematics,Physics,English">
        </div>
        <div class="form-section">
            <label>Facilities <span class="small">(comma-separated)</span></label>
            <input type="text" name="facilities[]" value="<?= htmlspecialchars($editing?str_replace(',','',$row['facilities']??''):'') ?>" placeholder="Auditorium,Medical,Playground">
        </div>
        <div class="form-section">
            <label>Faculty</label>
            <div id="faculty">
    ...
    ?>
    <h2>All Schools</h2>
    <input type="text" id="searchBar" placeholder="Search by School Name..." onkeyup="filterTable()">
