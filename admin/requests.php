...
// categories.php - City Explorer Category Page with Counts
...
// ✅ Mapping between cat_image and real table names
$tableMap = [
    "hospital"    => "t_hospital",
    "school"      => "t_schools",
    "restaurant"  => "t_restaurants",
    "college"       => "t_colleges",
    "pg"        => "t_pgs",
    "historical_place"        => "t_h_p",
    // 👉 Add more mappings as per your DB
];
...
if(mysqli_num_rows($result3)) {
    while($fetch=mysqli_fetch_assoc($result3)) {
        $tableName = isset($tableMap[$fetch['cat_image']]) ? $tableMap[$fetch['cat_image']] : null;
        $count = 0;
        if ($tableName) {
...
?>
                <a href="req_<?php echo $fetch['cat_image'];?>.php" class="category-card">
                    ...
                    <div class="category-count"><?php echo $count; ?> Request</div>
                </a>
<?php 
    }   
}
