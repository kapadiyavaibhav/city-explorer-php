...
if (isset($_POST['remove_gallery_img']) && isset($_POST['id'])) {
    ...
    $res = mysqli_query($conn, "SELECT gallery FROM h_p WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    $gallery = str2arr($row['gallery']);
    $gallery = array_filter($gallery, fn($img) => $img !== $remove_img);
    $gallery_str = arr2str($gallery);
    mysqli_query($conn, "UPDATE h_p SET gallery='$gallery_str' WHERE id=$id");
...
if (isset($_POST['save'])) {
    ...
    $old_gallery = [];
    if ($id) {
        $fetch = mysqli_query($conn, "SELECT img,gallery FROM h_p WHERE id=$id");
    ...
    if ($id) {
        ...
        $sql = "UPDATE h_p SET ".implode(',',$sets)." WHERE id=$id";
        ...
        $sql = "INSERT INTO h_p ($fieldstr) VALUES ('$values')";
        mysqli_query($conn, $sql);
...
if (isset($_GET['del'])) {
    $id = intval($_GET['del']);
    $res = mysqli_query($conn, "SELECT img,gallery FROM h_p WHERE id=$id");
    $row = mysqli_fetch_assoc($res);
    if ($row) {
        if (!empty($row['img'])) {
    ...
    mysqli_query($conn, "DELETE FROM h_p WHERE id=$id");
...
if ($edit_id) {
    $res = mysqli_query($conn, "SELECT * FROM h_p WHERE id=$edit_id");
    $row = mysqli_fetch_assoc($res);
    if ($row) $editing = true;
}

$res = mysqli_query($conn, "SELECT * FROM h_p ORDER BY id DESC");
$rows = [];
...
<html>
<head>
    <title>Historical Place Admin</title>
    <style>
...
<div class="container">
    ...
    <form method="post" enctype="multipart/form-data" id="hpForm">
        ...
        <div class="form-section">
            ...
            <input type="text" name="features[]" value="<?= htmlspecialchars($editing?str_replace(',',',',$row['features']??''):'') ?>" placeholder="Heritage,UNESCO Site,Gardens">
        </div>
        <div class="form-section">
            <label>Guides/Contacts (comma-separated)</label>
            <input type="text" name="guides[]" value="<?= htmlspecialchars($editing?str_replace(',',',',$row['guides']??''):'') ?>" placeholder="Mr. X,Ms. Y">
        </div>
        <div class="form-section">
    ...
    <input type="text" id="searchBar" placeholder="Search by Place Name..." onkeyup="filterTable()">
    <table id="placeTable">
        <tr>
