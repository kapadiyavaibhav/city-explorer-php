...
if ($id === 0) { die('Invalid college ID'); }
$res = mysqli_query($conn, "SELECT * FROM colleges WHERE id = $id");
$college = mysqli_fetch_assoc($res);
if (!$college) { die('College not found'); }
$college['courses'] = $college['courses'] ? explode(',', $college['courses']) : [];
$college['departments'] = $college['departments'] ? explode(',', $college['departments']) : [];
$college['facilities'] = $college['facilities'] ? explode(',', $college['facilities']) : [];
$college['gallery'] = $college['gallery'] ? explode(',', $college['gallery']) : [];
$college['faculty'] = $college['faculty'] ? array_map(function($f){
    list($n,$d,$e)=array_pad(explode('|',$f),3,'');
    return ['name'=>$n,'dept'=>$d,'exp'=>$e];
}, explode(',',$college['faculty'])) : [];
$college['scholarships'] = $college['scholarships'] ? explode(',', $college['scholarships']) : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?= htmlspecialchars($college['name']) ?> - College Details</title>
    ...
    <style>
        ...
        .college-logo { width: 150px; height: 150px; border-radius: 50%; object-fit: cover; margin-right: 28px; border: 3px solid #e0e7ef; background: #fff; }
        .college-title { font-size: 2.3rem; color: #2d6ca2; letter-spacing: 1px; margin-bottom: 4px; }
        .college-address a { color: #3887ff; text-decoration: none; font-size: 1.08rem; }
        .info-list, .departments-list, .courses-list, .facilities-list, .faculty-list, .scholarships-list, .gallery-list { list-style: none; padding: 0; margin: 0; }
        .info-list li, .departments-list li, .courses-list li, .facilities-list li, .scholarships-list li, .gallery-list li { margin-bottom: 6px; font-size: 1.08rem; }
    ...
    color: #1d4d72 !important;
    margin-bottom: 14px !important;
    border-left: 6px solid #4cb6e0 !important;
    padding-left: 12px !important;
}
        /* Navbar tabs style (SAME AS PG DETAILS) */
        .navbar-tabs {
            display: flex;
            gap: 34px;
            font-size: 1.08rem;
        ...
        @media (max-width: 600px) {
            ...
            .college-logo { margin-bottom: 10px; }
...
<div class="containerr">
    <div class="headerr">
        <img src="images/<?= htmlspecialchars($college['img']) ?>" alt="College Logo" class="college-logo">
        <div>
            <div class="college-title"><?= htmlspecialchars($college['name']) ?></div>
            <div class="college-address">
                <?= htmlspecialchars($college['address']) ?> &#x1F5FA;
                <a href="https://maps.google.com/?q=<?= urlencode($college['name']) ?>,<?= urlencode($college['city_name']) ?>" target="_blank">View on Map</a>
            </div>
            <ul class="info-list">
                <li>📞 <b>Contact:</b> <a href="tel:<?= htmlspecialchars($college['contact']) ?>" onclick="event.stopPropagation();"><?= htmlspecialchars($college['contact']) ?></a></li>
                <li>✉️ <b>Email:</b> <a href="mailto:<?= htmlspecialchars($college['email']) ?>"><?= htmlspecialchars($college['email']) ?></a></li>
                <li>🌐 <b>Website:</b> <?php if($college['website']): ?><a href="<?= htmlspecialchars($college['website']) ?>" target="_blank"><?= htmlspecialchars($college['website']) ?></a><?php endif; ?></li>
                <li><b>Established:</b> <?= htmlspecialchars($college['established']) ?></li>
            </ul>
        </div>
    </div>
    <!-- NAVBAR TABS -->
    <nav>
        <div class="navbar-tabs" id="navbarTabs">
            <button class="active" data-tab="facultyTab">Faculty</button>
            <button data-tab="coursesTab">Courses & Facilities</button>
    ...
    <p></p>
    <!-- TAB SECTIONS -->
    <div class="section tab-section active" id="facultyTab">
        <div class="section-title">Faculty</div>
        <div class="faculty-list">
            <?php foreach($college['faculty'] as $fac): ?>
                <div class="faculty-card">
                    <div class="name"><?= htmlspecialchars($fac['name']) ?></div>
                    <div class="department"><?= htmlspecialchars($fac['dept']) ?></div>
                    <div>🧑‍🏫 <?= htmlspecialchars($fac['exp']) ?> years experience</div>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    <div class="section tab-section" id="coursesTab">
        ...
        <div class="section-title" style="margin-top:22px;">Departments</div>
        <ul class="departments-list">
            <?php foreach($college['departments'] as $dept): ?>
                <li>• <?= htmlspecialchars($dept) ?></li>
            <?php endforeach; ?>
        </ul>
        <div class="section-title" style="margin-top:22px;">Facilities</div>
        <ul class="facilities-list">
            <?php foreach($college['facilities'] as $fac): ?>
                <li><?= htmlspecialchars($fac) ?></li>
            <?php endforeach; ?>
            <?php if($college['hostel']) echo '<li>Hostel</li>'; ?>
            <?php if($college['transport']) echo '<li>Transport</li>'; ?>
            <?php if($college['library']) echo '<li>Library</li>'; ?>
            <?php if($college['sports']) echo '<li>Sports</li>'; ?>
            <?php if($college['cafeteria']) echo '<li>Cafeteria</li>'; ?>
            <?php if($college['disabled_friendly']) echo '<li>Disabled Friendly</li>'; ?>
        </ul>
    </div>
    <div class="section tab-section" id="admissionTab">
        <div class="section-title">Admission Process</div>
        <div class="description"><?= htmlspecialchars($college['admission_process']) ?></div>
        <div class="section-title" style="margin-top:22px;">Scholarships</div>
        <ul class="scholarships-list">
            <?php foreach($college['scholarships'] as $sch): ?>
    ...
    <div class="section tab-section" id="galleryTab">
        ...
        <ul class="gallery-list">
            <?php foreach($college['gallery'] as $img): ?>
                <li>
